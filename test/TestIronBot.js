...
const IronBot = require("../src/bots/IronBot");

const player = new IronBot();
...
test("getNextMove", function(t) {
  ...
  t.equal(
    player.getNextMove(["e2e4", "a7a6", "f1c4", "a8a7"]),
    "c4f7",
    "check is played when available"
  );
  t.equal(
    player.getNextMove(["e2e4", "g8f6", "f2f4"]),
    "f6e4",
    "capture is played"
  );
  t.notOk(
